...
}},beforeunload:{amazonOriginal:null,setup:function(data,namespaces,eventHandle){if(jQuery.isWindow(this)){var f=function(){};
if(typeof this.onbeforeunload==="function"){f=jQuery.event.special.beforeunload.amazonOriginal=this.onbeforeunload;
}this.onbeforeunload=function(){var args=Array.prototype.slice.call(arguments);
f.apply(this,args);
eventHandle.apply(this,args);
};
}},teardown:function(namespaces,eventHandle){this.onbeforeunload=jQuery.event.special.beforeunload.amazonOriginal;
}}}};
...
}var rinlinejQuery=/ jQuery\d+="(?:\d+|null)"/g,rleadingWhitespace=/^\s+/,rxhtmlTag=/<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig,rtagName=/<([\w:]+)/,rtbody=/<tbody/i,rhtml=/<|&#?\w+;/,rnocache=/<(?:script|object|embed|option|style)/i,rchecked=/checked\s*(?:[^=]|=\s*.checked.)/i,rscriptType=/\/(java|ecma)script/i,rcleanScript=/^\s*<!(?:\[CDATA\[|\-\-)|[\]\-]{2}>\s*$/g,wrapMap={option:[1,"<select multiple='multiple'>","</select>"],legend:[1,"<fieldset>","</fieldset>"],thead:[1,"<table>","</table>"],tr:[2,"<table><tbody>","</tbody></table>"],td:[3,"<table><tbody><tr>","</tr></tbody></table>"],col:[2,"<table><tbody></tbody><colgroup>","</colgroup></table>"],area:[1,"<map>","</map>"],_default:[0,"",""]};
...
}var body=doc.body,win=getWindow(doc),ieTouch=navigator.msMaxTouchPoints>0,clientTop=docElem.clientTop||body.clientTop||0,clientLeft=docElem.clientLeft||body.clientLeft||0,scrollTop=!ieTouch&&win.pageYOffset||jQuery.support.boxModel&&docElem.scrollTop||body.scrollTop,scrollLeft=!ieTouch&&win.pageXOffset||jQuery.support.boxModel&&docElem.scrollLeft||body.scrollLeft,top=box.top+scrollTop-clientTop,left=box.left+scrollLeft-clientLeft;
...
}else{patchJQuery(window.jQuery);
}})();
(function(){var setupJQuery=function(jQuery){if(jQuery){jQuery.ajaxSettings.traditional=true;
}};
if(window.amznJQ&&amznJQ.initJQuery){var initJQuery=amznJQ.initJQuery;
amznJQ.initJQuery=function(){initJQuery();
setupJQuery(window.jQuery);
};
}else{setupJQuery(window.jQuery);
}})();
